...
public partial class LevelTree : CsgBox3D
{
	...
	private void GenerateTree(Random rnd)
	{
		...
		for(int depthLayer = 0; depthLayer < _maxDepth; depthLayer ++)
		{
			...
			AddParentConnectionWhereNull(root, depthLayer, existingConnections);
		}
	...
	private void AddParentConnectionWhereNull(LevelNode root, int depthLayer, HashSet<(int parent, int child)> existingConnections)
	{
		for (int nodeIndex = 0; nodeIndex < MAX_NODE_COUNT; nodeIndex++)
		{
			...
			/*while (parent == null)
			{
				...
				if (IsCross(parentIndex, nodeIndex, existingConnections))
				{
					parentIndexRef++;
					continue;
				}
